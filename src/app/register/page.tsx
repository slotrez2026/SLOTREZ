...
export default function RegisterPage() {
  ...
  function pasteNoSpaces(setter: (v: string) => void) {
    return (e: React.ClipboardEvent) => {
      e.preventDefault();
      setter(noSpaces(e.clipboardData.getData("text")));
    };
  }

  const strength = password.length === 0 ? null : password.length < 8 ? "weak" : password.length < 12 ? "good" : "strong";
  const strengthColor = strength === "weak" ? "#ef4444" : strength === "good" ? "#f59e0b" : "#3b6d11";
  const strengthLabel = strength === "weak" ? "Too short — min 8 characters" : strength === "good" ? "Good password" : "Strong password ✓";
  ...
  async function handleSignup() {
    ...
    if (role === "biz" && !businessName.trim()) { setError("Please enter your business name."); return; }
    ...
    const { data, error: signUpError } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          ...
          business_name: role === "biz" ? businessName.trim() : null,
        },
      },
    });
    ...
    if (signUpError) {
      ...
      } else if (signUpError.message.toLowerCase().includes("already") || signUpError.message.toLowerCase().includes("registered") || signUpError.message.toLowerCase().includes("exists")) {
        setEmailExists(true);
      } else {
    ...
    if (data.user && data.user.identities && data.user.identities.length === 0) {
      setEmailExists(true); setLoading(false); return;
    }
    ...
    if (data.user) {
      await supabase.from("profiles").upsert({
        ...
        full_name: `${firstName.trim()} ${lastName.trim()}`.trim(),
        role: role === "biz" ? "owner" : "client", onboarding_complete: false,
      });
      if (role === "biz" && businessName.trim()) {
        await supabase.from("businesses").insert({ owner_id: data.user.id, name: businessName.trim(), plan: "free" });
      }
    }
  ...
  async function handleSendOtp() {
    ...
    setPhoneE164(data.phone); setLoading(false); setStep("verify");
    setResendTimer(30);
  ...
  async function handleVerify() {
    ...
    setLoading(true); setError("");
    const res = await fetch("/api/auth/verify-otp", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone: phoneE164, token: code }) });
    const data = await res.json();
    if (data.error) { setError("Invalid code. Please try again."); setDigits(["", "", "", "", "", ""]); setLoading(false); return; }
    const { data: { user } } = await supabase.auth.getUser();
    if (user) await supabase.from("profiles").update({ phone: phoneE164 }).eq("id", user.id);
          ...
          <div style={{ marginBottom: "1rem" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <input type="tel" inputMode="tel" autoComplete="tel" value={phone}
                ...
                placeholder="2015550192" style={{ ...baseInput, flex: 1 }} />
            </div>
            {phone.length > 0 && phone.length < 10 && <p style={{ fontSize: 11, color: "#f59e0b", marginTop: 4 }}>Enter your full 10-digit number</p>}
            {phone.length === 10 && <p style={{ fontSize: 11, color: "#3b6d11", marginTop: 4 }}>✓ Looks good</p>}
          </div>
      ...
      <div style={{ background: "#fff", border: "0.5px solid #e5e7eb", borderRadius: 12, padding: "2rem", width: "100%", maxWidth: 440 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: ".85rem" }}>
          <div>
            ...
            <input autoComplete="given-name" autoCapitalize="words" value={firstName}
              onChange={(e) => { setFirstName(e.target.value); setError(""); }} placeholder="Alex" style={baseInput} />
          </div>
          <div>
            <label style={{ fontSize: 12, color: "#6b7280", display: "block", marginBottom: 3 }}>Last name</label>
            <input autoComplete="family-name" autoCapitalize="words" value={lastName}
              onChange={(e) => setLastName(e.target.value)} placeholder="Rivera" style={baseInput} />
          </div>
        </div>
        ...
        <div style={{ marginBottom: ".85rem" }}>
          ...
          <input
            ...
            value={email}
            onChange={(e) => { setEmail(noSpaces(e.target.value)); setError(""); setEmailExists(false); }}
            onKeyDown={blockSpace}
            onPaste={pasteNoSpaces(setEmail)}
            placeholder="you@example.com" style={baseInput} />
        </div>
        ...
        {emailExists && (
          <div style={{ background: "#faeeda", border: "0.5px solid #f59e0b", borderRadius: 8, padding: "10px 12px", fontSize: 12, color: "#854f0b", marginBottom: "1rem", lineHeight: 1.6 }}>
            ...
            <Link href="/forgot-password" style={{ color: "#854f0b", fontWeight: 600, textDecoration: "underline" }}>Reset your password</Link>{" or "}
            <Link href="/login" style={{ color: "#854f0b", fontWeight: 600, textDecoration: "underline" }}>log in instead</Link>.
          </div>
        )}
        ...
        <div style={{ marginBottom: ".85rem" }}>
          ...
          {strength && <p style={{ fontSize: 11, color: strengthColor, marginTop: 4 }}>{strengthLabel}</p>}
        </div>

        {role === "biz" && (
          <div style={{ marginBottom: ".85rem" }}>
            <label style={{ fontSize: 12, color: "#6b7280", display: "block", marginBottom: 3 }}>Business name *</label>
            <input autoComplete="organization" value={businessName}
              onChange={(e) => { setBusinessName(e.target.value); setError(""); }}
              placeholder="Your shop or studio name" style={baseInput} />
          </div>
        )}
        ...
        <p style={{ textAlign: "center", marginTop: "1rem", fontSize: 13, color: "#6b7280" }}>
          ...
          <Link href="/login" style={{ color: "#2563eb", textDecoration: "none", fontWeight: 500 }}>Log in</Link>
        </p>
      </div>
    </div>
  );
